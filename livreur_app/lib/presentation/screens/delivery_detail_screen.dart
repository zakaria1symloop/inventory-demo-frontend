        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                  ...
                  child: Column(
                    children: [
                      ...order.items.take(3).map((item) {
                        ...
                        final total = item.quantityConfirmed;
                        final cartons = ppp > 1 ? total ~/ ppp : 0;
                        final remain = ppp > 1 ? total % ppp : 0;
                        return Padding(
              ...
              // Print button for delivered/partial orders
              if (!isPending && (order.isDelivered || order.isPartial)) ...[
                const Divider(),
                Align(
                  alignment: Alignment.centerLeft,
                  child: TextButton.icon(
                    onPressed: () => PrintUtils.printOrderReceipt(context, order),
      ...
      children: [
        ...
        if (products.isEmpty)
          ...
          ...products.entries.map((entry) {
            ...
            final ppp = data['ppp'] as int? ?? 1;
            final remaining = data['loaded'] - data['delivered'] - data['returned'];
            final loadedCartons = ppp > 1 ? (data['loaded'] as int) ~/ ppp : 0;
            final loadedRemain = ppp > 1 ? (data['loaded'] as int) % ppp : data['loaded'] as int;
            return Card(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (ppp > 1) ...[
                      ...
                      Text(
                        'محمّل: $loadedCartons كرتون${loadedRemain > 0 ? ' + $loadedRemain قطعة' : ''} = ${data['loaded']} قطعة',
...
class _StockBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        '$label: ${formatQtyPieces(value, piecesPerPackage)}',
