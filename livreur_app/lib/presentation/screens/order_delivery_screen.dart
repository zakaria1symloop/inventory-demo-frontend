...
class _OrderDeliveryScreenState extends ConsumerState<OrderDeliveryScreen> {
        ...
        if (itemsList != null && itemsList.isNotEmpty) {
          ...
          for (var itemJson in itemsList) {
            final item = _ProductItem(
              ...
              quantityConfirmedPieces: _parseNum(itemJson['quantity_confirmed']),
  ...
  double _itemDeliveredAmount(_ProductItem item) {
    final deliveredPcs = _deliveredTotalPieces(item);
    final confirmedPcs = item.confirmedTotalPieces;
    if (confirmedPcs <= 0) return 0;
    // Use subtotal (which includes discount) proportionally
    return item.subtotal * (deliveredPcs / confirmedPcs);
  }

  double _itemReturnedAmount(_ProductItem item) {
    final returnedPcs = _returnedTotalPieces(item);
    final confirmedPcs = item.confirmedTotalPieces;
    if (confirmedPcs <= 0) return 0;
    return item.subtotal * (returnedPcs / confirmedPcs);
  }

  double get totalDeliveredAmount {
    double total = 0;
    for (var item in _products) {
      total += _itemDeliveredAmount(item);
    }
    return total;
  }
  ...
  double get totalReturnedAmount {
    double total = 0;
    for (var item in _products) {
      total += _itemReturnedAmount(item);
    }
    return total;
  }
              ...
              children: [
                ...
                _qtyButton(Icons.add, deliveredQty < item.confirmedCartons || (deliveredQty == item.confirmedCartons && deliveredPcs < item.confirmedExtraPieces), () {
                  setState(() {
                    _hasModified = true;
                    deliveredQuantities[item.productId] = deliveredQty + 1;
                    // Cap pieces if we exceed confirmed total
      ...
      if (hasReturns && _products.isNotEmpty) {
        final items = _products.map((item) {
          ...
          return {
            ...
            'quantity_delivered': deliveredTotalPcs,
            'quantity_returned': returnedTotalPcs,
...
class _ProductItem {
  ...
  final int quantityConfirmedPieces; // total pieces confirmed (from API)
  ...
  _ProductItem({
    ...
    required this.quantityConfirmedPieces,
    required this.unitPrice,
    required this.discount,
    required this.subtotal,
  });

  // Convert pieces to cartons + extra pieces for display
  int get confirmedCartons => piecesPerPackage > 1
      ? quantityConfirmedPieces ~/ piecesPerPackage
      : quantityConfirmedPieces;
  int get confirmedExtraPieces => piecesPerPackage > 1
      ? quantityConfirmedPieces % piecesPerPackage
      : 0;
  int get confirmedTotalPieces => quantityConfirmedPieces;
}
