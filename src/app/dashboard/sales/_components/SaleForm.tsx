...
export default function SaleForm({ saleId = null, onSuccess, onCancel }: SaleFormProps) {
  ...
  return (
    <div>
      ...
      {/* Quick Entry Modal — stock-aware. Shows available stock prominently
         and warns the user inline when their requested quantity exceeds
         what's on hand (red dot + ring on the qty input + disabled Add). */}
      {quickEntryModal.show && quickEntryModal.product && (() => {
        ...
        // Stock awareness
        const availablePieces = getProductStock(product);
        const requestedPieces = (quickEntryModal.quantity || 0) * ppp;
        const overStock = requestedPieces > availablePieces;
        const outOfStock = availablePieces <= 0;
        const stockDot = outOfStock
          ? 'metric-dot-red'
          : availablePieces < ppp * 5
          ? 'metric-dot-orange'
          : 'metric-dot-green';
        ...
        const canConfirm = !isBelowCost && !overStock && !outOfStock && (quickEntryModal.quantity || 0) > 0;
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl flex flex-col overflow-hidden w-full max-w-[480px] max-h-[calc(100vh-3rem)] pointer-events-auto border border-gray-200 dark:border-gray-700">
                <header className="px-5 py-3 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <div className="mt-1 flex items-center gap-1.5 text-[12px] font-medium text-gray-600 dark:text-gray-400">
                    ...
                    {outOfStock
                      ? t('stock.outOfStock')
                      : `${t('saleForm.availableCol')}: ${formatStockQty(availablePieces, ppp)}`}
                  </div>
                </header>
                <main className="flex-1 overflow-y-auto p-5 space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`input w-full text-center text-[15px] font-semibold tnum transition-colors ${
                        overStock || outOfStock
                          ? '!border-red-400 focus:!border-red-500 focus:!ring-red-100 !bg-red-50/40 dark:!bg-red-900/10'
                          : ''
                      }`}
                      min="1"
                      autoFocus
                    />
                    {(overStock || outOfStock) && (
                      <p className="inline-flex items-center gap-1.5 text-[11px] mt-1.5 font-medium text-red-700 dark:text-red-400">
                        <span className="metric-dot metric-dot-red" aria-hidden />
                        {outOfStock
                          ? t('stock.outOfStock')
                          : t('saleForm.availableQtyOnly', { qty: formatStockQty(availablePieces, ppp) })}
                      </p>
                    )}
                  </div>

                  <div>
                    <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('saleForm.sellingPricePerPiece')}</label>
                    <input
                      ref={quickPriceRef}
                      type="number"
                      ...
                      className={`input w-full text-center text-[15px] font-semibold tnum transition-colors ${
                        isBelowCost ? '!border-red-400 focus:!border-red-500 focus:!ring-red-100 !bg-red-50/40 dark:!bg-red-900/10' : ''
                      }`}
                      min="0"
                      step="0.01"
                    />
                ...
                <footer className="flex gap-2 px-5 py-3 border-t border-gray-200 dark:border-gray-700">
                  <button
                    ...
                    disabled={!canConfirm}
                    className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2 text-[13px] font-semibold rounded-md text-white bg-emerald-600 hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {t('saleForm.addEnter')}
                  </button>
                  <button
      ...
      {onCancel ? (
        <div className="mb-4">
          <h1 className="text-[20px] md:text-[22px] font-semibold text-gray-900 dark:text-white tracking-tight leading-tight">
            {isEditMode ? t('saleForm.editSaleInvoice') : t('saleForm.newSaleInvoice')}
          </h1>
          <p className="mt-1 text-[13px] text-gray-500 dark:text-gray-400">
            {isEditMode ? t('saleForm.editSubtitle') : t('saleForm.newSubtitle')}
          </p>
        </div>
      ) : (
        <PageHeader
          title={isEditMode ? t('saleForm.editSaleInvoice') : t('saleForm.newSaleInvoice')}
          subtitle={isEditMode ? t('saleForm.editSubtitle') : t('saleForm.newSubtitle')}
          breadcrumb={[
            { label: t('sales.title'), href: '/dashboard/sales' },
            { label: isEditMode ? t('saleForm.editSaleInvoice') : t('saleForm.newSaleInvoice') },
          ]}
        />
      )}
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2 space-y-4">
            {/* Basic Info */}
            <div className="surface-pro" data-tour="sf-info">
              <div className="px-4 py-2.5 border-b border-gray-200/80 dark:border-gray-700">
                <h2 className="text-[13px] font-semibold text-gray-700 dark:text-gray-200">{t('saleForm.invoiceInfo')}</h2>
              </div>
              <div className="p-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="relative">
                  <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1">{t('saleForm.client')}</label>
                  <input
                    ref={clientSearchRef}
                    type="text"
                    value={clientSearch}
                ...
                <div>
                  <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1">{t('saleForm.warehouse')}</label>
                  <select
                    value={warehouseId}
                ...
                <div>
                  <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1">{t('saleForm.dateLabel')}</label>
                  <DateInput
                    value={date}
              ...
              {/* Client Debt Info */}
              {clientId && (
                <div className="mt-4 p-3 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60">
                  {loadingDebt ? (
                    <div className="text-center text-[12px] text-gray-500 dark:text-gray-400">{t('saleForm.loadingDebt')}</div>
                  ) : clientDebt ? (
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        <span className="inline-flex items-center gap-1.5 text-[12px] font-medium text-gray-700 dark:text-gray-300">
                          <span className={`metric-dot ${clientDebt.balance > 0 ? 'metric-dot-red' : 'metric-dot-green'}`} aria-hidden />
                          {t('saleForm.previousDebt')}
                        </span>
                        <span className="text-[14px] font-semibold text-gray-900 dark:text-gray-100 tnum">
                      ...
                      {(() => {
                        ...
                        return categoryName ? (
                          <div className="flex items-center justify-between text-[12px]">
                            <span className="text-gray-500 dark:text-gray-400">{t('saleForm.priceCategory')}</span>
                            <span className="inline-flex items-center gap-1.5 text-[12px] font-medium text-gray-700 dark:text-gray-300">
                      ...
                      {clientDebt.unpaid_orders && clientDebt.unpaid_orders.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                          <div className="text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-2">{t('saleForm.unpaidInvoices')}</div>
                          <div className="max-h-24 overflow-y-auto space-y-1">
                            {clientDebt.unpaid_orders.slice(0, 5).map((order) => (
                              <div key={order.id} className="flex justify-between text-[12px]">
                                <span className="text-gray-500 dark:text-gray-400">{order.reference}</span>
                                <span className="tnum text-gray-900 dark:text-gray-100">{formatCurrency(order.due_amount)}</span>
                              </div>
                            ))}
                          </div>
                        </div>
            ...
            {/* Product Search */}
            <div className="surface-pro" data-tour="sf-products">
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-gray-200/80 dark:border-gray-700">
                <div className="flex items-center gap-2">
                  <h2 className="text-[13px] font-semibold text-gray-700 dark:text-gray-200">{t('saleForm.addProducts')}</h2>
                  {items.length > 0 && (
                    <span className="inline-flex items-center gap-1 text-[11px] font-medium text-gray-500 dark:text-gray-400 tnum">
                      ...
                      {t('saleForm.productCount', { count: items.length })}
                    </span>
                  )}
                </div>
                {/* Search Mode Toggle */}
                <div className="flex items-center gap-2.5">
                  <span className={`text-[12px] ${searchMode === 'barcode' ? 'text-gray-700 dark:text-gray-200 font-medium' : 'text-gray-400'}`}>{t('saleForm.barcodeMode')}</span>
                  <button
                    type="button"
                  ...
                  <span className={`text-[12px] ${searchMode === 'name' ? 'text-gray-700 dark:text-gray-200 font-medium' : 'text-gray-400'}`}>{t('saleForm.nameMode')}</span>
                </div>
              </div>
              ...
              <div className="px-4 pt-3 pb-0 mb-3">
                {searchMode === 'barcode' ? (
                  <div>
                    <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1">{t('saleForm.barcodeSearch')}</label>
                    <input
                      ref={barcodeInputRef}
                      type="text"
                      value={barcodeInput}
                  ...
                  <div className="relative">
                    <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1">{t('saleForm.nameSearch')}</label>
                    <input
                          ...
                          filteredProducts.slice(0, 10).map((product, index) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full p-3 text-start border-b border-gray-100 dark:border-gray-700 last:border-b-0 transition-colors ${isHighlighted ? 'bg-blue-100 dark:bg-blue-900/40' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                              >
              ...
              {/* Items Table */}
              <div className="px-4 pb-4" data-tour="sf-items">
                <div className="table-pro-wrap">
                <table className="table-pro compact">
                  <thead>
                    <tr>
                      <th className="text-center w-10">{t('saleForm.numberCol')}</th>
                      <th>{t('saleForm.designationCol')}</th>
                      <th className="text-center w-[120px]">{t('saleForm.cartonPieceCol')}</th>
                      <th className="text-center w-20">{t('saleForm.unitCol')}</th>
                      <th className="text-center w-[100px]">{t('saleForm.countCol')}</th>
                      <th className="text-center w-24">{t('saleForm.availableCol')}</th>
                      <th className="text-center w-[120px]">{t('saleForm.unitPriceCol')}</th>
                      <th className="text-center w-[100px]">{t('saleForm.discountCol')}</th>
                      <th className="text-end w-[110px]">{t('saleForm.amountCol')}</th>
                  ...
                  <tbody>
                    {items.length === 0 ? (
                      <tr>
                        <td colSpan={10} className="text-center py-12">
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <p className="text-[13px] text-gray-500 dark:text-gray-400 font-semibold">{t('saleForm.noProductsYet')}</p>
                            <p className="text-[11px] text-gray-400 dark:text-gray-500">{t('saleForm.useBarcodeOrName')}</p>
                          </div>
                        </td>
                      </tr>
                    ) : (
                      items.map((item, index) => {
                        ...
                        const totalPiecesForStock = (item.quantity * ppp) + (item.extra_pieces || 0);
                        const overStock = totalPiecesForStock > item.available_stock;
                        return (
                          <tr key={index} className={`group ${isBelowCostPrice ? 'bg-red-50/40 dark:bg-red-900/10' : ''}`}>
                            ...
                            <td>
                              ...
                              {isBelowCostPrice && (
                                <div className="inline-flex items-center gap-1.5 mt-1 text-[11px] font-medium text-gray-700 dark:text-gray-300">
                                  <span className="metric-dot metric-dot-red" aria-hidden />
                                  {t('saleForm.belowCostLabel', { price: formatCurrency(item.cost_price) })}
                                </div>
                              )}
                            </td>
                            ...
                            <td className="text-center tnum">
                              <span className={`inline-flex items-center gap-1.5 text-[12px] font-medium ${overStock ? 'text-gray-700 dark:text-gray-300' : 'text-gray-700 dark:text-gray-300'}`}>
                            ...
                            <td className="px-2 py-2">
                              <input
                                ...
                                className={`input w-full !px-2 text-center text-[13px] py-1 font-semibold tnum ${isBelowCostPrice ? 'border-red-500' : ''}`}
                                min="0"
                                step="0.01"
                              />
                              {item.pieces_per_package > 1 && (
                            ...
                            <td className="text-end tnum t-strong">
            ...
            <div className="surface-pro p-4">
              <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('saleForm.notes')}</label>
              <textarea
                value={note}
          ...
          {/* Sidebar - Summary */}
          <div>
            <div className="surface-pro sticky top-24" data-tour="sf-summary">
              <div className="px-4 py-2.5 border-b border-gray-200/80 dark:border-gray-700">
                <h2 className="text-[13px] font-semibold text-gray-700 dark:text-gray-200">{t('saleForm.invoiceSummary')}</h2>
              </div>

              <div className="p-4 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-[13px] text-gray-500 dark:text-gray-400">{t('saleForm.productsTotal', { count: items.length })}</span>
                  <span className="font-semibold text-gray-900 dark:text-gray-100 tnum">{formatCurrency(totalAmount)}</span>
                </div>

                {/* Compact 3-column: Discount / Tax / Shipping */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-400 dark:text-gray-500 mb-1">{t('saleForm.discountLabel')}</label>
                    <input
                      type="number"
                      value={discount}
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-400 dark:text-gray-500 mb-1">{t('saleForm.taxLabel')}</label>
                    <input
                      type="number"
                      value={tax}
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-400 dark:text-gray-500 mb-1">{t('saleForm.shippingLabel')}</label>
                    <input
                      type="number"
                      value={shipping}
                  ...
                  <div>
                    <label className="block text-[11px] font-semibold text-gray-400 dark:text-gray-500 mb-1">{locale === 'ar' ? 'الطابع %' : 'Timbre %'}</label>
                    <input
                      type="number"
                      value={timbre}
                ...
                <div className="flex justify-between items-center p-3 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                  <span className="text-[13px] font-semibold text-gray-700 dark:text-gray-200">{t('saleForm.finalTotal')}</span>
                ...
                <div className="p-3 rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800">
                  <label className="block text-[12px] font-medium text-gray-700 dark:text-gray-300 mb-1.5">{t('saleForm.paidAmount')}</label>
                  <input
                    ref={paidAmountRef}
                    type="number"
                    value={paidAmount}
                ...
                {clientId && clientDebt ? (
                  <div className="p-3.5 bg-slate-50 dark:bg-gray-700/50 border border-slate-200/60 dark:border-gray-600 rounded-xl space-y-2">
                    ...
                    <div className="p-2.5 bg-white dark:bg-gray-800 rounded-lg border border-slate-100 dark:border-gray-700 mb-2 space-y-1">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-gray-500 dark:text-gray-400">{t('saleForm.thisInvoice')}</span>
                        <span className="font-bold tabular-nums">{formatCurrency(grandTotal)}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-gray-500 dark:text-gray-400">{t('saleForm.plusPreviousDebt')}</span>
                      ...
                      <div className="flex justify-between items-center text-sm font-bold">
                        <span className="text-slate-700 dark:text-slate-300">{t('saleForm.equalsTotal')}</span>
                        <span className="text-blue-700 dark:text-blue-400 tabular-nums">{formatCurrency(grandTotal + previousDebt)}</span>
                      </div>
                    </div>

                    <div className="text-[11px] font-bold text-slate-600 dark:text-slate-400 mb-2">{t('saleForm.paymentDistribution', { amount: formatCurrency(currentPaidAmount) })}</div>

                    {currentPaidAmount > 0 ? (
                      <>
                        {/* Applied to current sale */}
                        <div className="flex justify-between items-center text-sm">
                          <span className="text-gray-500 dark:text-gray-400">{t('saleForm.deductedFromInvoice')}</span>
                          <span className="font-semibold text-emerald-600 tabular-nums">{formatCurrency(appliedToCurrentSale)}</span>
                        </div>

                        {/* Applied to previous debt */}
                        {appliedToPreviousDebt > 0 && (
                          <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-500 dark:text-gray-400">{t('saleForm.deductedFromPreviousDebt')}</span>
                      ...
                      <div className="text-sm text-gray-400 text-center py-1">{t('saleForm.noPaidAmountEntered')}</div>
                    )}

                    {/* Remaining */}
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-500 dark:text-gray-400">{t('saleForm.invoiceRemaining')}</span>
                      <span className={`font-semibold tabular-nums ${remainingFromSale > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                        {formatCurrency(remainingFromSale)}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-500 dark:text-gray-400">{t('saleForm.previousDebtRemaining')}</span>
                    ...
                    <div className="flex justify-between items-center p-2 bg-white dark:bg-gray-800 rounded-lg border border-slate-100 dark:border-gray-700">
                      <span className="font-bold text-slate-700 dark:text-slate-300 text-sm">{t('saleForm.totalDebtAfterCollection')}</span>
                  ...
                  <div className="p-3 bg-gray-50 dark:bg-gray-700/50 border border-gray-200/60 dark:border-gray-600 rounded-xl">
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-gray-500 dark:text-gray-400">{t('saleForm.remainingFromInvoice')}</span>
                      <span className={`font-black tabular-nums ${remainingFromSale > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
                        {formatCurrency(remainingFromSale)}
                      </span>
                    </div>
                  </div>
                ...
                <div className="space-y-2 pt-1" data-tour="sf-save">
                  <button
                    ...
                    className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 text-[13px] font-semibold rounded-md text-white bg-emerald-600 hover:bg-emerald-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSaving ? t('saleForm.saving') : t('saleForm.saveInvoice')}
                  ...
                  {onCancel ? (
                    ...
                    <Link href="/dashboard/sales" className="w-full inline-flex items-center justify-center px-4 py-2 text-[13px] font-semibold rounded-md border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                      {t('saleForm.cancel')}
                    </Link>
                  )}
                </div>
