...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (payment === 'success') {
      ...
      setPaymentMsg({ type: 'success', text: plan ? `تمت ترقية خطتك إلى ${plan} بنجاح!` : 'تمت عملية الدفع بنجاح! سيتم تحديث خطتك.' });
      router.replace('/dashboard');
    } else if (payment === 'failed') {
      setPaymentMsg({ type: 'error', text: 'فشلت عملية الدفع. حاول مرة أخرى أو تواصل مع الدعم.' });
      router.replace('/dashboard');
    } else if (payment === 'pending') {
      const paymentId = searchParams.get('payment_id');
      const plan = searchParams.get('plan');
      setPaymentMsg({ type: 'success', text: 'جاري التحقق من حالة الدفع...' });
      router.replace('/dashboard');
          ...
          for (let i = 0; i < 10; i++) {
            ...
            try {
              ...
              if (res.data.status === 'paid') {
                setPaymentMsg({ type: 'success', text: plan ? `تمت ترقية خطتك إلى ${plan} بنجاح!` : 'تمت عملية الدفع بنجاح!' });
                return;
              } else if (res.data.status === 'failed') {
                setPaymentMsg({ type: 'error', text: 'فشلت عملية الدفع. حاول مرة أخرى.' });
                return;
              }
            } catch { break; }
          }
          setPaymentMsg({ type: 'success', text: plan ? `تم إرسال طلب الترقية إلى ${plan}. سيتم تحديث خطتك قريباً.` : 'تم إرسال الدفع. سيتم تحديث خطتك عند التأكيد.' });
        };
        pollStatus();
      }
    }
