...
export default function VanSessionDetailPage() {
  ...
  if (!session) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/dashboard/van-sales" className="btn btn-primary mt-4">
          العودة للقائمة
        </Link>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={fetchSession} className="btn btn-ghost">
          ...
          {session.status === 'active' && (
            <>
              ...
              <button onClick={handleCancel} className="btn btn-error btn-outline">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="card p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm text-gray-500">السائق</div>
              <div className="font-medium">{session.livreur?.name || '-'}</div>
              <div className="text-sm text-gray-400">
                {session.vehicle?.plate_number || 'بدون مركبة'}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm text-gray-500">المستودع</div>
              <div className="font-medium">{session.warehouse?.name || '-'}</div>
        ...
        <div className="card p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="font-medium text-green-600">{formatCurrency(session.total_sales)}</div>
              <div className="text-sm text-gray-400">{session.sales_count} عملية</div>
            </div>
          </div>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="font-medium">
                {session.start_time ? formatTime(session.start_time) : 'لم يبدأ'}
              </div>
              {session.end_time && (
                <div className="text-sm text-gray-400">انتهى: {formatTime(session.end_time)}</div>
              )}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div className="card p-4 bg-blue-50">
          <div className="text-blue-600 text-sm font-medium">قيمة التحميل</div>
          <div className="text-xl font-bold text-blue-700">{formatCurrency(session.total_loaded_value)}</div>
        </div>
        <div className="card p-4 bg-green-50">
          <div className="text-green-600 text-sm font-medium">المبيعات</div>
          <div className="text-xl font-bold text-green-700">{formatCurrency(session.total_sales)}</div>
        </div>
        <div className="card p-4 bg-emerald-50">
          <div className="text-emerald-600 text-sm font-medium">المحصل</div>
          <div className="text-xl font-bold text-emerald-700">{formatCurrency(session.total_collected)}</div>
        </div>
        <div className="card p-4 bg-red-50">
          <div className="text-red-600 text-sm font-medium">الآجل</div>
          <div className="text-xl font-bold text-red-700">{formatCurrency(session.total_credit)}</div>
        </div>
        <div className="card p-4 bg-gray-50">
          <div className="text-gray-600 text-sm font-medium">قيمة المرتجع</div>
          <div className="text-xl font-bold text-gray-700">{formatCurrency(session.total_returned_value)}</div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        <div className="flex border-b">
          <button
            ...
            className={`px-6 py-3 font-medium ${activeTab === 'products' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700'}`}
          >
            المنتجات ({session.items?.length || 0})
          </button>
          <button
            onClick={() => setActiveTab('sales')}
            className={`px-6 py-3 font-medium ${activeTab === 'sales' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700'}`}
          >
            المبيعات ({session.sales?.length || 0})
          </button>
          <button
            onClick={() => setActiveTab('returns')}
            className={`px-6 py-3 font-medium ${activeTab === 'returns' ? 'text-blue-600 border-b-2 border-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-700'}`}
          >
            المرتجعات ({session.returns?.length || 0})
          </button>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50">
                    ...
                    <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">محمّل</th>
                    <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">مباع</th>
                    <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">مرتجع</th>
                    <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">متبقي</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={remaining > 0 ? 'text-blue-600 font-medium' : 'text-gray-400'}>
                            {formatQty(remaining, ppp)}
                          </span>
                        </td>
                      </tr>
                    );
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">المدفوع</th>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">السبب</th>
                      <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">قابل للإرجاع</th>
                      <th className="px-4 py-3 text-center text-sm font-medium text-gray-600">الحالة</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                      ...
                      <tr key={ret.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-center">
                          {ret.returnable_to_stock ? (
                            ...
                            <span className="text-red-600">لا (خسارة)</span>
                          )}
                        </td>
                        <td className="px-4 py-3 text-center">
                          {ret.processed ? (
