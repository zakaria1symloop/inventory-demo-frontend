...
export default function EditVanSession() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (session.status !== 'preparing') {
        toast.error(t('vanSessions.cannotEditAfterStart'));
      ...
      if (session.items && session.items.length > 0) {
        setItems(session.items.map((item: { product_id: number; quantity_loaded: number; product?: { name: string; retail_price: number } }) => {
          ...
          return {
            ...
            product_name: item.product?.name || prod?.name || `${t('vanSessions.productFallback')} #${item.product_id}`,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('vanSessions.sessionUpdated'));
      router.push(`/dashboard/van-sessions/${id}`);
    } catch (error: unknown) {
      const err = error as { response?: { data?: { message?: string } } };
      toast.error(err.response?.data?.message || t('vanSessions.errorUpdatingSession'));
    } finally {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white">{t('vanSessions.editSessionTitle')} {sessionRef}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">{t('vanSessions.editSessionDescription')}</p>
        </div>
        <Link href={`/dashboard/van-sessions/${id}`} className="btn btn-secondary">{t('vanSessions.back')}</Link>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="card space-y-4">
              ...
              >
                {isSubmitting ? <span className="spinner w-4 h-4"></span> : t('vanSessions.saveChanges')}
              </button>
            </div>
          </div>
